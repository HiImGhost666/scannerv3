...
def show_about(parent):
    ...
    try:
        ...
        # Estilo según brandbook
        palette = about_window.palette()
        palette.setColor(QPalette.Window, QColor(240, 240, 240))
        about_window.setPalette(palette)

        # Contenido
        title_label = QLabel("Herramienta de Inventariado y Monitoreo de Red")
        title_label.setFont(QFont('Wix Madefor Text', 12, QFont.Bold))
        title_label.setAlignment(Qt.AlignCenter)

        version_label = QLabel("Versión 3.0")
        version_label.setAlignment(Qt.AlignCenter)

        separator = QFrame()
        separator.setFrameShape(QFrame.HLine)
        separator.setFrameShadow(QFrame.Sunken)
        separator.setStyleSheet(f"color: {QColor(193, 0, 22).name()};")
        ...
        desc_label = QLabel(
            ...
            "y monitorear la red en tiempo real."
        )
        desc_label.setAlignment(Qt.AlignCenter)
        desc_label.setWordWrap(True)

        copyright_label = QLabel("© 2025 LÄBERIT")
        ...
        close_btn.setStyleSheet(f"""
            QPushButton {{
                background-color: {QColor(193, 0, 22).name()};
                color: white;
                border: none;
                padding: 8px 20px;
        ...
        """)
        close_btn.clicked.connect(about_window.close)
        ...
        # Ejecutar el loop de Qt si no hay uno activo
        if not parent:
            sys.exit(app.exec_())

    except Exception as e:
        logger.exception(f"Error al mostrar ventana Acerca de: {e}")
        if parent:
            from tkinter import messagebox
            messagebox.showerror("Error", f"No se pudo mostrar la ventana:\n{e}")